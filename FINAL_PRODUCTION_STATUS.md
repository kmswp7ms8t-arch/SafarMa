...
Version: **8.5.0 · V16**  
Release code commit: `2775ac4b04278a90d35a7cc4489d2da930f7bc56`
...
- Two frontend editions are published from the same codebase:
  - private personal gift edition for Amir and Sanaz;
...
- Personal entry points are marked `noindex,nofollow,noarchive` to reduce search-engine exposure.
- The public edition contains no birthday or personal-preset content and uses a separate install manifest.
- The public edition includes canonical, Open Graph, Twitter, robots and sitemap metadata.
- Without a backend, ordinary public visitors do not see a technical server-configuration prompt; the built-in planner remains available with a launch-status notice.
...
- V16 frontend CI passed JavaScript, both manifests, sitemap XML, public/personal separation, SEO, launch-safe UX, privacy and secret checks.
...
A public connected backend cannot be created without authorization inside a hosting account. The repository is ready for the Render Blueprint flow:
...
The Blueprint already configures the GitHub Pages CORS origin, model, persistent database disk, generated session-signing secret, rate limits, turn limits and `/ready` health check.
